...
function browsersync() {
  browserSync.init({
    server: { baseDir: 'prod/' }, // Указываем папку сервера
...
function html() {
  ...
  .pipe(dest('prod/')) // Выгрузим на прод
...
function styles() {
  ...
  .pipe(dest('prod/css/')) // Выгрузим результат в прод
  .pipe(browserSync.stream()) // Триггерим Browsersync
}
...
function scripts() {
  ...
  .pipe(uglify()) // Сжимаем
  .pipe(dest('prod/js/')) // Выгружаем готовый файл
  .pipe(browserSync.stream()) // Триггерим Browsersync
}

function images() {
	return src('assets/img/**/*') // Берём все изображения из папки источника
	.pipe(newer('prod/img')) // Проверяем, было ли сжато изображение ранее
	.pipe(imagemin()) // Сжимаем и оптимизируем изображеня
	.pipe(dest('prod/img/')) // Выгружаем оптимизированные изображения в папку назначения
  .pipe(browserSync.stream()) // Триггерим Browsersync
}

function cleanimg() {
	return del('prod/img/**/*', { force: true }) // Удаляем все изображения прода
}
function cleanstyles() {
	return del('prod/css/**/*', { force: true }) // Удаляем все стили прода
}
function cleanscripts() {
	return del('prod/js/**/*', { force: true }) // Удаляем все скрипты прода
}
function cleanhtml() {
	return del('prod/*.html', { force: true }) // Удаляем html прода
}
function cleanall() {
	cleanimg();
  cleanstyles();
...
// Build
exports.build = series(pugtohtml, html, prep, styles, scripts, images);
